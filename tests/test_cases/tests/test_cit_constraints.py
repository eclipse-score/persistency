...
class TestConstraintConfiguration(CommonScenario):
    ...
    def test_constraint_configuration(
        ...
        - Compile-time constraints: KVS_MAX_SNAPSHOTS constant in source code (C++ only)
        """
        # Skip compile-time test for Rust - it doesn't have a hardcoded constant like C++ KVS_MAX_SNAPSHOTS
        if constraint_type == "compile_time" and version == "rust":
            pytest.skip("Rust doesn't have a compile-time constant like C++ KVS_MAX_SNAPSHOTS")
